...
#### 진입 방식
- 설정 화면에서 앱 버전 텍스트 **5번 연속 탭** → 개발자 모드 진입
- `__DEV__` 플래그로 감싸서 프로덕션 빌드에서 자동 제거

#### 시뮬레이터 기능

**① 출석 시뮬레이터**
- 진행 중인 챌린지 목록 표시 → 탭하면 GPS 이벤트 없이 즉시 출석 처리
- 체류 시간 인증 ON인 챌린지는 체류 시간 직접 입력 가능 (30분 대기 없이 테스트)
- 내부적으로 `challengeStore.recordVisit()`을 직접 호출

**② 시간 점프**
- "다음 주로 이동" 버튼 → 시스템 시간이 아닌, 챌린지 로직 내부의 기준 시점을 오프셋
- `devStore.timeOffsetDays` 값을 조절하여 챌린지 로직에서 참조하는 시간만 변경
- 사용 예: 1주차 출석 완료 → 다음 주 점프 → 콤보 확인 → 8주 후 졸업까지 몇 분 내 테스트
...
**③ 상태 강제 세팅**
- 특정 챌린지의 콤보 수, 찬스 수, 주차를 직접 입력하여 원하는 상태로 세팅
- 챌린지를 즉시 졸업 상태로 전환 가능
- 사용 예: 찬스 0개 + 목표 미달 상태를 만들어서 콤보 리셋 화면 확인

### 단위 테스트 (Jest)

위치/UI 없이 순수 판정 로직만 테스트. `challengeStore`의 함수를 직접 호출하고 결과를 검증.

#### 출석 판정 테스트

```typescript
describe('출석 판정', () => {
  test('요일 지정 OFF: 아무 요일이나 카운트', () => {
    const challenge = createChallenge({ weeklyGoal: 3, daySpecific: false });
    const result = recordVisit(challenge, { dayOfWeek: 'TUE' });
    expect(result.counted).toBe(true);
  });

  test('요일 지정 ON: 지정 안 한 요일은 카운트 안 됨', () => {
    const challenge = createChallenge({
      weeklyGoal: 3, daySpecific: true, days: ['MON', 'WED', 'FRI']
    });
    const result = recordVisit(challenge, { dayOfWeek: 'TUE' });
    expect(result.counted).toBe(false);
  });

  test('체류 시간 미충족: 카운트 안 됨', () => {
    const challenge = createChallenge({ dwellTime: 30 });
    const result = recordVisit(challenge, { dwellMinutes: 15 });
    expect(result.counted).toBe(false);
  });
});
```
...
describe('콤보 시스템', () => {
  test('주간 목표 달성 → 콤보 +1', () => {
    const challenge = createChallenge({ weeklyGoal: 3 });
    completeWeek(challenge, { visits: 3 });
    expect(challenge.combo).toBe(1);
  });

  test('주간 실패 + 찬스 있음 → 콤보 유지, 찬스 -1', () => {
    const challenge = createChallenge({ combo: 2, chances: 1 });
    completeWeek(challenge, { visits: 1 });
    expect(challenge.combo).toBe(2);
    expect(challenge.chances).toBe(0);
  });

  test('주간 실패 + 찬스 없음 → 콤보 리셋', () => {
    const challenge = createChallenge({ combo: 4, chances: 0 });
    completeWeek(challenge, { visits: 1 });
    expect(challenge.combo).toBe(0);
  });

  test('3콤보 달성 → 방어권 추가 증정', () => {
    const challenge = createChallenge({ combo: 2, chances: 0 });
    completeWeek(challenge, { visits: 3 });
    expect(challenge.combo).toBe(3);
    expect(challenge.chances).toBe(1);
  });
});
```
...
```typescript
describe('졸업 시스템', () => {
  test('4주 챌린지 4주차 완료 → 졸업', () => {
    const challenge = createChallenge({ duration: 4, currentWeek: 4 });
    completeWeek(challenge, { visits: 3 });
    expect(challenge.status).toBe('graduated');
  });

  test('반복 모드 ON + 졸업 → 콤보 유지하며 새 사이클', () => {
    const challenge = createChallenge({
      duration: 3, currentWeek: 3, repeat: true, combo: 5
    });
    completeWeek(challenge, { visits: 3 });
    expect(challenge.status).toBe('active');
    expect(challenge.currentWeek).toBe(1);
    expect(challenge.combo).toBe(6);
  });
});
```
...
| 테스트 수단 | 확인 대상 | 장점 |
|------------|-----------|------|
| 개발자 모드 | UI 플로우, 화면 전환, 애니메이션, 푸시 알림 | 실제 앱 위에서 동작, 시각적 확인 가능 |
| 단위 테스트 (Jest) | 출석 판정, 콤보 계산, 찬스 소모, 졸업 조건 | 0.1초 내 실행, 엣지 케이스 촘촘히 커버 |
