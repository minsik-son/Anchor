...
export async function createAlarm(input: CreateAlarmInput): Promise<number> {
    ...
    const result = await database.runAsync(
        `INSERT INTO alarms (title, latitude, longitude, radius, sound_uri) VALUES (?, ?, ?, ?, ?)`,
        [input.title, input.latitude, input.longitude, input.radius ?? 500, input.sound_uri ?? null]
    );
...
export async function updateAlarm(id: number, updates: Partial<CreateAlarmInput & { is_active: boolean }>): Promise<void> {
    const database = getDatabase();
    const fields: string[] = [];
    const values: any[] = [];
